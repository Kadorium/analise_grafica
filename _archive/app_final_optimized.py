...
def endpoint_wrapper(endpoint_name_fallback: str):
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            request_obj = next((a for a in args if isinstance(a, Request)), kwargs.get('request', None))
            current_endpoint_name = get_request_metadata(request_obj) or endpoint_name_fallback
            ...
            try:
                ...
                log_endpoint(f"{current_endpoint_name} - REQUEST SUCCESS", elapsed_time=f"{elapsed:.2f}s", status_code=status)
                return response
            except Exception as e:
                elapsed = time.time() - start_time
                log_endpoint(f"{current_endpoint_name} - REQUEST ERROR", elapsed_time=f"{elapsed:.2f}s", error=str(e), traceback=traceback.format_exc())
...
def ensure_processed_data():
    if CACHED_DATA['processed_data'] is None:
...
# Global state
AppState.uploaded_data = None
AppState.processed_data = None
AppState.backtester = None
AppState.current_config = cfg.get_all_config()
...
async def upload_file(file: Optional[UploadFile] = None):
    ...
    AppState.uploaded_data = data_loader.load_csv()
    logger.info(f"Data loaded successfully: {AppState.uploaded_data.shape}")
    
    for col in AppState.uploaded_data.columns:
        if 'unnamed' in col.lower() and AppState.uploaded_data[col].isna().all():
            AppState.uploaded_data = AppState.uploaded_data.drop(columns=[col])
    
    sample_data_for_preview = stringify_df_dates(AppState.uploaded_data.head(5))
    
    response_data = {
        "message": "File processed successfully" if default_file_used else "File uploaded successfully",
        "data_shape": AppState.uploaded_data.shape,
        "data_sample": sample_data_for_preview.to_dict('records'),
        "columns": list(AppState.uploaded_data.columns)
    }
...
async def arrange_data_endpoint(file: UploadFile = File(...)):
    ...
    AppState.uploaded_data = arranged_data.copy()
    AppState.processed_data = arranged_data.copy()
    
    sample_data_for_preview = stringify_df_dates(arranged_data.head(5))
    
    date_range = {}
    if 'date' in AppState.processed_data.columns and pd.api.types.is_datetime64_any_dtype(AppState.processed_data['date']):
        date_min = AppState.processed_data['date'].min()
        date_max = AppState.processed_data['date'].max()
...
async def add_indicators(indicator_config: IndicatorConfig):
    ...
    log_endpoint("POST /api/add-indicators - DETAILS", 
                ...
                data_shape=AppState.processed_data.shape if AppState.processed_data is not None else "None")
    ...
    missing_cols = check_required_columns(AppState.processed_data, required_cols)
    if missing_cols:
        return JSONResponse(
            status_code=400,
    ...
    existing_base_columns = [col for col in base_columns if col in AppState.processed_data.columns]
    data_for_indicators = AppState.processed_data[existing_base_columns].copy()
        
    data_with_indicators = combine_indicators(data_for_indicators, indicators_dict)
    AppState.processed_data = data_with_indicators
        
    excluded_columns = base_columns + ['ticker', 'index'] # Define base columns explicitly
    indicator_columns = [col for col in AppState.processed_data.columns if col not in excluded_columns]
        
    summary = ""
    if indicator_columns:
        try:
            indicator_summary_df = create_indicator_summary(AppState.processed_data, last_n_periods=1)
...
async def plot_indicators(plot_config: PlotConfig):
    ...
    image_base64 = plot_price_with_indicators(AppState.processed_data, plot_dict)
...
async def save_config_endpoint(config_data: Dict[str, Any], request: Request):
    ...
    AppState.current_config.update(config_data)
    ...
    with open(config_file_path, 'w') as f:
        json.dump(AppState.current_config, f, indent=4)
...
async def load_config_endpoint(config_file: str, request: Request):
    ...
    AppState.current_config.update(loaded_config_data)
    log_endpoint(f"{request.method} {request.url.path} - LOADED", file=config_file)
    return {"message": "Configuration loaded successfully", "config": AppState.current_config}
...
async def export_results(format_type: str, request: Request):
    ...
    if AppState.backtester is None or not AppState.backtester.results:
    ...
    if format_type.lower() == 'json':
        ...
        AppState.backtester.save_results(file_to_send_path)
        media_type_str = "application/json"
    elif format_type.lower() == 'csv':
        file_to_send_name = f"backtest_results_{timestamp}.csv"
        file_to_send_path = os.path.join(results_dir_export, file_to_send_name)
        all_results_df = pd.DataFrame()
        if isinstance(AppState.backtester.results, dict):
            for strategy_name, result_data in AppState.backtester.results.items():
...
async def get_current_config(request: Request):
    , AppState.processed_data
    
    if AppState.processed_data is not None:
        indicator_columns = [col for col in AppState.processed_data.columns 
                           if col not in REQUIRED_COLUMNS]
        if 'indicators' not in AppState.current_config: AppState.current_config['indicators'] = {}
        AppState.current_config['indicators']['available_indicators'] = indicator_columns
    
    return AppState.current_config
...
async def data_status(request: Request):
    , AppState.processed_data
    return {
        "uploaded": AppState.uploaded_data is not None,
        "processed": AppState.processed_data is not None,
        "shape": AppState.processed_data.shape if AppState.processed_data is not None else None
    }
...
async def debug_info(request: Request):
    ...
    if AppState.uploaded_data is not None:
        uploaded_data_summary = {
            "shape": AppState.uploaded_data.shape, "columns": list(AppState.uploaded_data.columns),
            "memory_usage": f"{AppState.uploaded_data.memory_usage(deep=True).sum() / (1024**2):.2f} MB",
            "sample_rows_count": len(AppState.uploaded_data.head(3))
        }
    ...
    if AppState.processed_data is not None:
        ...
        if 'date' in AppState.processed_data.columns and pd.api.types.is_datetime64_any_dtype(AppState.processed_data['date']):
            min_d, max_d = AppState.processed_data['date'].min(), AppState.processed_data['date'].max()
        ...
        processed_data_summary = {
            "shape": AppState.processed_data.shape, "columns": list(AppState.processed_data.columns),
            "memory_usage": f"{AppState.processed_data.memory_usage(deep=True).sum() / (1024**2):.2f} MB",
            "date_range": date_range_processed,
            "has_indicators": len([c for c in AppState.processed_data.columns if c not in REQUIRED_COLUMNS]) > 0
        }
...
async def analyze_day_of_week(request: Request):
    ...
    ensure_processed_data() return JSONResponse(status_code=400, content={"success": False, "message": "No data."})
    
    log_endpoint(f"{request.method} {request.url.path} - START_ANALYSIS")
    dow_returns_df, fig_dow = day_of_week_returns(AppState.processed_data, plot=True)
...
async def analyze_monthly(request: Request):
    ...
    log_endpoint(f"{request.method} {request.url.path} - START_ANALYSIS")
    monthly_rets_df, fig_monthly = monthly_returns(AppState.processed_data, plot=True)
...
async def analyze_volatility(request: Request):
    ...
    log_endpoint(f"{request.method} {request.url.path} - START_ANALYSIS")
    dow_vol_df, fig_vol = day_of_week_volatility(AppState.processed_data, plot=True)
...
async def create_heatmap(request: Request):
    ...
    log_endpoint(f"{request.method} {request.url.path} - START_ANALYSIS")
    fig_heatmap = calendar_heatmap(AppState.processed_data)
...
async def get_seasonality_summary(request: Request):
    ...
    log_endpoint(f"{request.method} {request.url.path} - START_ANALYSIS")
    fig_summary, results_data = seasonality_summary(AppState.processed_data)
