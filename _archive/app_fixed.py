...
# Fun̤̣o para gerar logs formatados de forma consistente
def log_endpoint(endpoint_name, **kwargs):
    """
    Gera logs formatados de forma consistente para os endpoints.
    
    Args:
        endpoint_name: Nome do endpoint para identifica̤̣o
        **kwargs: Dados adicionais a serem logados (par̢metros, resultados, etc.)
    """
    separator = "=" * 50
    log_lines = [f"\n{separator}", f"ENDPOINT: {endpoint_name}", f"TIMESTAMP: 
{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"]
    
    for key, value in kwargs.items():
        # Formata̤̣o especial para DataFrames
        if isinstance(value, pd.DataFrame):
            log_lines.append(f"{key.upper()}: shape={value.shape}, 
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                elif isinstance(response, dict): # Heuristic for direct dict returns from Pydantic 
models etc.
                    # Try to infer status from content, default to 200 if looks like success
                    if response.get("success") is False and "message" in response: # Check if it's 
an error dict
...
def check_required_columns(df: Optional[pd.DataFrame], required_cols: List[str]) -> List[str]:
    ...
    if df is None:
        ...
        # Returning required_cols might be misleading if df being None is an expected state before 
data load.
        # Let's adjust to return empty if df is None, assuming check is on existing df.
        return [] if df is not None else required_cols # Modified logic: if df is None, all 
...
def normalize_signals_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'signal' not in signals_df.columns:
        ...
        if 'position' in signals_df.columns:
            ...
            logger.warning("No signal or position columns found to derive from. Defaulting to 
'hold'.")
            signals_df['signal'] = 'hold'
        signals_df['signal'] = signals_df['signal'].fillna('hold')


    if 'signal' in signals_df.columns and signals_df['signal'].dtype in [np.int64, np.float64, 
'int64', 'float64']:
    ...
    logger.info(f"Signal normalization complete. Signal counts: 
{signals_df['signal'].value_counts().to_dict() if 'signal' in signals_df else 'N/A'}")
    return signals_df

# Import our modules
from data.data_loader import DataLoader
from indicators.indicator_utils import combine_indicators, plot_price_with_indicators, 
...
from indicators.seasonality import day_of_week_returns, monthly_returns, day_of_week_volatility, 
calendar_heatmap, seasonality_summary
...
def log_optimization_request(request_data, extra_info=None, error=None, traceback_info=None):
    """
    Appends optimization request data, extra info, and errors to a log file for debugging and 
traceability.
    """
    import json
    import traceback as tb
    ...
    if 'param_ranges' in request_data:
        ...
        for param_name, param_values in request_data['param_ranges'].items():
            ...
            if isinstance(param_values, list) and len(param_values) > 10:
                formatted_params[param_name] = f"{len(param_values)} values from 
{min(param_values)} to {max(param_values)}"
            else:
    ...
    if extra_info is not None:
        ...
        if 'default_performance' in extra_info and 'optimized_performance' in extra_info:
            perf_comparison = {}
            for metric in ['sharpe_ratio', 'total_return_percent', 'max_drawdown_percent', 
'win_rate_percent']:
                if metric in extra_info['default_performance'] and metric in 
extra_info['optimized_performance']:
                    default_val = extra_info['default_performance'][metric]
                    opt_val = extra_info['optimized_performance'][metric]
                    perf_comparison[metric] = {
                        'default': round(default_val, 4) if isinstance(default_val, (int, float)) 
else default_val,
                        'optimized': round(opt_val, 4) if isinstance(opt_val, (int, float)) else 
opt_val,
        ...
        if 'default_params' in extra_info and 'optimized_params' in extra_info:
            ...
            all_params = set(list(extra_info['default_params'].keys()) + 
list(extra_info['optimized_params'].keys()))
        ...
        if 'all_results' in extra_info:
            top_results_summary = []
            for i, result in enumerate(extra_info['all_results'][:min(3, 
len(extra_info['all_results']))]):
                result_summary = {
                    'rank': i + 1,
                    'params': result['params'],
                    'score': round(result['value'], 4) if isinstance(result['value'], (int, float)) 
else result['value']
                }
...
async def upload_file(file: Optional[UploadFile] = None):
    ...
    if file:
        log_endpoint("POST /api/upload - DETAILS", file_name=file.filename, 
content_type=file.content_type)
        ...
        if not os.path.exists(default_file_path):
            log_endpoint("POST /api/upload - DETAILS", error=f"Default file not found: 
{default_file_path}")
            return JSONResponse(
                status_code=404,
                content={"message": f"Default file {default_file_path} not found. Please upload a 
file."}
            )
    ...
    response_data = {
        "message": "File processed successfully" if default_file_used else "File uploaded 
successfully",
...
async def arrange_data_endpoint(file: UploadFile = File(...)):
    ...
    date_range = {}
    if 'date' in PROCESSED_DATA.columns and 
pd.api.types.is_datetime64_any_dtype(PROCESSED_DATA['date']):
...
async def process_data():
    ...
    if UPLOADED_DATA is None:
        return JSONResponse(
            ...
            content={"success": False, "message": "No data uploaded. Please upload a CSV file 
first."}
        )
    ...
    try:
        ...
        if len(cleaned_data) == 0:
            logger.warning("Empty dataset after cleaning. Attempting recovery with European date 
format...")
            ...
            if len(data_copy) > 0:
                ...
                return JSONResponse(
                    status_code=400,
                    content={"success": False, "message": "Could not process data. All rows were 
...
pd.api.types.is_datetime64_any_dtype(PROCESSED_DATA['date']):
        ...
        logger.error(f"Error during data processing/recovery: 
{str(e_recovery)}\n{traceback.format_exc()}")
        # This error will be caught by the endpoint_wrapper if re-raised,
        # or return a specific JSONResponse here.
        # For consistency, let the wrapper handle it by re-raising or returning a specific known 
error.
        # However, if we return a JSONResponse here, the wrapper's error handling won't run.
        # It's better to raise a specific error or let the original exception propagate to the 
wrapper.
...
async def add_indicators(indicator_config: IndicatorConfig):
    ...
    if PROCESSED_DATA is None:
        return JSONResponse(
            status_code=400,
            content={"success": False, "message": "No processed data available. Please upload and 
process data first."}
        )
    ...
    if missing_cols:
        return JSONResponse(
            status_code=400,
            content={"success": False, "message": f"Missing required columns in data: {', 
'.join(missing_cols)}"}
        )
    ...
    if indicator_columns:
        try:
            ...
            summary = f"<div class='alert alert-info'><strong>Indicators added:</strong> {', 
'.join(indicator_columns)}</div>"
            ...
            summary = f"<div class='alert alert-warning'><strong>Indicators added</strong> but 
could not create summary: {str(e_summary)}</div>"
...
async def run_backtest(strategy_config: StrategyConfig, backtest_config: BacktestConfig, request: 
Request):
    ...
    if backtest_config.start_date:
        filtered_data = filtered_data[filtered_data['date'] >= 
...
pd.to_datetime(backtest_config.end_date)]
    ...
    if 'signal' in signals_df.columns:
        ...
        if buy_count < 3 or sell_count < 3:
            logger.warning(f"Few signals found (Buy: {buy_count}, Sell: {sell_count}), attempting 
to add more test signals based on MA crossover.")
            if 'close' in signals_df.columns:
                if 'sma_20' not in signals_df.columns:
                    signals_df['sma_20'] = signals_df['close'].rolling(window=20, 
min_periods=1).mean()
                if 'sma_50' not in signals_df.columns:
                    signals_df['sma_50'] = signals_df['close'].rolling(window=50, 
min_periods=1).mean()
                ...
                if not signals_df.empty : # Check if df is not empty after dropna
                    ...
                    buy_condition = (signals_df['sma_20'] > signals_df['sma_50']) & 
(signals_df['sma_20'].shift(1) <= signals_df['sma_50'].shift(1))
                    sell_condition = (signals_df['sma_20'] < signals_df['sma_50']) & 
(signals_df['sma_20'].shift(1) >= signals_df['sma_50'].shift(1))
                    
                    # Apply signals where conditions are met and current signal is 'hold'
                    signals_df.loc[buy_condition & (signals_df['signal'] == 'hold'), 'signal'] = 
'buy'
                    signals_df.loc[sell_condition & (signals_df['signal'] == 'hold'), 'signal'] = 
'sell'
                    logger.info(f"Added MA crossover test signals. New counts: Buy: 
{(signals_df['signal'] == 'buy').sum()}, Sell: {(signals_df['signal'] == 'sell').sum()}")

    results_metrics = calculate_performance_metrics(signals_df, 
                                           initial_capital=backtest_config.initial_capital, 
                                           commission=backtest_config.commission)
    
    # Ensure equity is added to signals_df for plotting (calculate_performance_metrics should 
return it)
    # This part seems redundant if calculate_performance_metrics already adds/returns equity 
correctly with signals_df
    # For now, assuming calculate_performance_metrics returns a modified signals_df or the equity 
series directly.
    # Let's assume calculate_performance_metrics returns a tuple (metrics_dict, 
signals_df_with_equity)
    
    # Rework based on calculate_performance_metrics potentially modifying signals_df or returning it
    # For now, assume metrics is a dict and signals_df might be modified in place or we use the one 
we have.
    # The original code implicitly assumed calculate_performance_metrics might not add 'equity' to 
the original signals_df.
    # Let's ensure 'equity' and 'cumulative_market_return' are in signals_df for 
plot_backtest_results.
    
    if 'equity' not in signals_df.columns and 'final_capital_series' in results_metrics: # Check if 
metrics returned it
        ...
        logger.warning("'equity' column not found in signals_df after performance calculation. 
Re-calculating for plot.")
        ...
        for i in range(len(signals_df)):
            ...
            if signal == 'buy' and current_pos == 0:
                ...
                # If holding, equity changes by price change relative to entry if we had a 'shares' 
concept
        ...
        signals_df['equity'] = temp_equity[1:] if len(temp_equity) > len(signals_df) else 
temp_equity # Align length
    ...
    chart_html = plot_backtest_results(signals_df, 
                                      strategy_name=strategy_config.strategy_type.replace("_", " 
").title(),
                                      initial_capital=backtest_config.initial_capital)
    ...
    CURRENT_CONFIG['strategy'] = {'type': strategy_config.strategy_type, 'parameters': 
strategy_config.parameters}
    indicator_columns = [col for col in data.columns if col not in ['date', 'open', 'high', 'low', 
'close', 'volume']]
    CURRENT_CONFIG['indicators'] = indicator_columns
    
    log_endpoint(f"{request.method} {request.url.path} - RESULT_SUMMARY", 
strategy_metrics=results_metrics.get('total_return_percent', 'N/A'))
...
# ... (calculate_performance_metrics, plot_backtest_results, extract_trades remain largely as is, 
ensure they don't have conflicting try-excepts for general errors)

@app.post("/api/optimize-strategy")
@endpoint_wrapper("POST /api/optimize-strategy")
async def optimize_strategy_endpoint(optimization_config: OptimizationConfig, background_tasks: 
BackgroundTasks, request: Request):
    ...
    log_endpoint(f"{request.method} {request.url.path} - DETAILS", 
optimization_cfg=optimization_config.dict())

    if PROCESSED_DATA is None:
        log_optimization_request(optimization_config.dict(), error="No processed data.")
        return JSONResponse(status_code=400, content={"success": False, "message": "No processed 
data."})
    ...
    comparison_result = {
        "status": "in_progress",
        "message": f"Optimization for {optimization_config.strategy_type} is running in the 
background."
    }
...
optimization_config.start_date, optimization_config.end_date)
            ...
            # Ensure default_signals has equity calculated
            if default_signals is not None and 'equity' not in default_signals.columns:
                logger.info("Calculating equity for default strategy signals")
                default_signals = calculate_equity_curve(default_signals, 10000.0, 0.001)
            ...
            optimized_strategy = create_strategy(optimization_config.strategy_type, 
**combined_params)
            optimized_result = backtester.run_backtest(optimized_strategy, 
optimization_config.start_date, optimization_config.end_date)
            optimized_performance = optimized_result['performance_metrics']
            optimized_signals = optimized_result['signals'] if 'signals' in optimized_result else 
None
            
            # Ensure optimized_signals has equity calculated
            if optimized_signals is not None and 'equity' not in optimized_signals.columns:
                logger.info("Calculating equity for optimized strategy signals")
                optimized_signals = calculate_equity_curve(optimized_signals, 10000.0, 0.001)
            ...
            # Add debug info for dataframes before chart generation
            logger.info("Debug info for default strategy signals:")
            
            logger.info("Debug info for optimized strategy signals:")
            
            if default_signals is not None and optimized_signals is not None:
                logger.info("Preparing comparison data for chart generation")
                
                # Align dates
                logger.info(f"Number of common dates for comparison: {len(common_dates)}")
                
                if len(common_dates) > 0:
                    logger.info(f"Date range for comparison: {common_dates[0]} to 
{common_dates[-1]}")
                    
                        default_signals.loc[common_dates, 'equity'].iloc[0],
                        default_signals.loc[common_dates, 'equity'].iloc[-1]
                    )
                        optimized_signals.loc[common_dates, 'equity'].iloc[0],
                        optimized_signals.loc[common_dates, 'equity'].iloc[-1]
                    )
            ...
            if default_signals is not None and optimized_signals is not None:
                try:
                    logger.info("Generating comparison chart between default and optimized equity 
curves")
                    
                    # Check if 'equity' column exists in both dataframes
                    if 'equity' not in default_signals.columns:
                        logger.warning("'equity' column not found in default_signals")
                        chart_html = "<div class='alert alert-warning'>Missing equity data for 
default strategy</div>"
                    elif 'equity' not in optimized_signals.columns:
                        logger.warning("'equity' column not found in optimized_signals")
                        chart_html = "<div class='alert alert-warning'>Missing equity data for 
optimized strategy</div>"
                    else:
                        # Align dates for both dataframes
                        if len(common_dates) == 0:
                            logger.warning("No common dates between default and optimized signals")
                            chart_html = "<div class='alert alert-warning'>No common dates for 
comparison chart</div>"
                        else:
                            default_equity = default_signals.loc[common_dates, 'equity'].tolist()
                            optimized_equity = optimized_signals.loc[common_dates, 
'equity'].tolist()
                            dates = [d.strftime('%Y-%m-%d') for d in common_dates]
                            
                            logger.info(f"Chart data points: {len(dates)}")
                            logger.info(f"Default equity points: {len(default_equity)}")
                            logger.info(f"Optimized equity points: {len(optimized_equity)}")
                            
                            # Generate chart HTML with unique ID to avoid conflicts
                            chart_id = f"comparison_chart_{int(time.time())}"
                            chart_html = f"""
...
document.querySelector('.comparison-chart-container').innerHTML = 
                                            '<div class="alert alert-danger">Chart container not 
found</div>';
                                        return;
                                    }}
                                    ...
                                    const defaultData = {json.dumps(default_equity)};
                                    const optimizedData = {json.dumps(optimized_equity)};
                                    
                                    // Log data sizes for debugging
                                    console.log("Chart data sizes - Labels:", labels.length, 
...
context.parsed.y.toFixed(2);
                                    ...
                                    // Store chart reference globally for chart type toggling
                                    window.comparisonChart = chart;
                                }} catch(error) {{
                                    console.error("Error creating chart:", error);
                                    document.querySelector('.comparison-chart-container').innerHTML 
+= 
                                        '<div class="alert alert-danger mt-2">Error creating chart: 
' + error.message + '</div>';
                                }}
                            }})();
                            </script>
                            """
                            logger.info("Comparison chart HTML generated successfully")
                except Exception as e:
                    logger.error(f"Error generating comparison chart: {str(e)}")
                    chart_html = f"<div class='alert alert-danger'>Error generating comparison 
chart: {str(e)}</div>"
            else:
                if default_signals is None:
                    logger.warning("Default signals not available for comparison chart")
                    chart_html = "<div class='alert alert-warning'>Default strategy signals not 
available</div>"
                elif optimized_signals is None:
                    logger.warning("Optimized signals not available for comparison chart")
                    chart_html = "<div class='alert alert-warning'>Optimized strategy signals not 
available</div>"
                else:
                    logger.warning("No data available for comparison chart")
                    chart_html = "<div class='alert alert-warning'>No data available for comparison 
chart</div>"

            # Make sure all required metrics are present in default_performance and 
optimized_performance
            def ensure_metrics_present(perf_dict):
                ...
                for metric in required_metrics:
                    if metric not in perf_dict or perf_dict[metric] is None or 
pd.isna(perf_dict[metric]):
...
f"optimization_{optimization_config.strategy_type}_{timestamp}.json")
...
f"optimization_{optimization_config.strategy_type}_{timestamp}.json")
                ...
                logger.error(f"Error saving optimization results: 
{str(e_file)}\n{traceback.format_exc()}")
        except Exception as e:
            error_msg = str(e)
    ...
    log_endpoint(f"{request.method} {request.url.path} - TASK_SCHEDULED", 
strategy=optimization_config.strategy_type)
...
async def get_optimization_results(strategy_type: str, request: Request):
    ...
    if OPTIMIZATION_STATUS["in_progress"] and OPTIMIZATION_STATUS["strategy_type"] == strategy_type:
        return {"status": "in_progress", "message": f"Optimization for {strategy_type} is still in 
progress"}
    ...
    try:
        files = [f for f in os.listdir(results_dir) if 
f.startswith(f"optimization_{strategy_type}_") and f.endswith(".json")]
    ...
    if not files:
        ...
        return {"status": "not_found", "message": f"No optimization results found for strategy type 
'{strategy_type}'"}
    ...
    try:
        ...
        return JSONResponse(content={
            ...
            "timestamp": datetime.fromtimestamp(os.path.getmtime(file_path)).strftime("%Y-%m-%d 
%H:%M:%S")
        })
    except Exception as e:
...
data."})
    ...
    if not strategy_types:
        return JSONResponse(status_code=400, content={"success": False, "message": "No strategy 
types."})
            
    valid_strategy_types_list = [s['type'] for s in AVAILABLE_STRATEGIES]
    invalid_strategies = [s_type for s_type in strategy_types if s_type not in 
valid_strategy_types_list]
    if invalid_strategies:
        return JSONResponse(status_code=422, content={"success": False, "message": f"Invalid types: 
{invalid_strategies}"})
    ...
    if results_comparison:
        try:
            best_strategy_name = max(
                ...
                key=lambda x: x[1].get('sharpe_ratio', -float('inf')) if isinstance(x[1], dict) 
else -float('inf')
            )[0]
    ...
    log_endpoint(f"{request.method} {request.url.path} - COMPARISON_SUMMARY", 
best_strategy=best_strategy_name, count=len(strategies_list))
    return {
...
async def load_config_endpoint(config_file: str, request: Request):
    ...
    if not os.path.exists(config_path_full):
        return JSONResponse(status_code=404, content={"success": False, "message": "Config file not 
found."})
...
async def export_results(format_type: str, request: Request):
    ...
    if BACKTESTER is None or not BACKTESTER.results:
        return JSONResponse(status_code=400, content={"success": False, "message": "No backtest 
results."})
    ...
    if format_type.lower() == 'json':
        ...
        if isinstance(BACKTESTER.results, dict):
            for strategy_name, result_data in BACKTESTER.results.items():
                if isinstance(result_data, dict) and 'backtest_results' in result_data and 
isinstance(result_data['backtest_results'], pd.DataFrame):
            ...
            if not all_results_df.empty:
                ...
                media_type_str = "text/csv"
                 ...
                 return JSONResponse(status_code=400, content={"success":False, "message": "No data 
to export to CSV."})
        else:
            return JSONResponse(status_code=400, content={"success":False, "message": "Results 
format not suitable for CSV export."})
    else:
        return JSONResponse(status_code=400, content={"success":False, "message": f"Unsupported 
format: {format_type}"})
...
async def debug_info(request: Request):
    system_info = {
        ...
        "processor": platform.processor(), "memory": f"{psutil.virtual_memory().total / 
(1024**3):.2f} GB",
    ...
    app_info = {
        ...
        "start_time_process": 
datetime.fromtimestamp(psutil.Process().create_time()).strftime('%Y-%m-%d %H:%M:%S'),
...
pd.api.types.is_datetime64_any_dtype(PROCESSED_DATA['date']):
        ...
        processed_data_summary = {
            ...
            "has_indicators": len([c for c in PROCESSED_DATA.columns if c not in ['date', 'open', 
'high', 'low', 'close', 'volume']]) > 0
        }

    data_info_summary = { "uploaded_data_summary": uploaded_data_summary, "processed_data_summary": 
processed_data_summary }

    log_endpoint(f"{request.method} {request.url.path} - DEBUG_INFO_ACCESS", 
...
async def analyze_day_of_week(request: Request):
    global PROCESSED_DATA
    if PROCESSED_DATA is None: return JSONResponse(status_code=400, content={"success": False, 
"message": "No data."})
    ...
    return JSONResponse(content={"success": True, "plot": img_str_b64, "data": 
dow_returns_df.to_dict('records')})
...
"message": "No data."})
    ...
    return JSONResponse(content={"success": True, "plot": img_str_b64, "data": 
monthly_rets_df.to_dict('records')})
...
def calculate_performance_metrics(signals_df, initial_capital=10000.0, commission=0.001):
    ...
    buy_signals_indices = [] # Storing indices for potential analysis, not directly used in metrics 
here
    ...
    for i in range(len(df)):
        ...
        if current_signal == 'buy':
            ...
            if position == 1:
                ...
                logger.info(f"SELL at {current_date} price: {exit_price:.2f}, profit: 
{trade_profit:.2f}")
    ...
    df['drawdown'] = (df['equity'].cummax() - df['equity']) / df['equity'].cummax().replace(0, 
np.nan) # Avoid div by zero if equity hits 0
...
total_loss can be negative
    
    daily_returns_series = df['equity'].pct_change().fillna(0)
    annual_volatility_calc = daily_returns_series.std() * (252 ** 0.5)
    sharpe_ratio_calc = annual_return_calc / annual_volatility_calc if annual_volatility_calc > 0 
else 0
...
def plot_backtest_results(signals_df, strategy_name='Strategy', initial_capital=10000.0):
    ...
    Args:
        signals_df (pd.DataFrame): DataFrame with backtest results (must include 'date', 'equity', 
'cumulative_market_return')
    ...
    if missing_plot_cols:
        logger.error(f"Missing columns for plotting: {missing_plot_cols}. Cannot generate equity 
curve.")
            ...
            try {{
                new Chart(ctx, {{
                    ...
                    options: {{
                        ...
                        plugins: {{ title: {{ display: true, text: 'Equity Curve' }}, tooltip: {{ 
mode: 'index', intersect: false }} }},
                        scales: {{
                            x: {{ display: true, title: {{ display: true, text: 'Date' }}, ticks: 
{{ maxTicksLimit: 12 }} }},
            ...
            }} catch (error) {{ console.error('Error creating equity curve chart ({chart_id}):', 
error); }}
        }};
...
def extract_trades(signals_df, commission=0.001, initial_capital=10000.0): # initial_capital not 
used here, but kept for signature consistency if desired
    """
    ...
    for i in range(len(df)):
        ...
        if current_signal == 'buy' and position == 0:
            ...
            if entry_price != 0: # Ensure there was an entry to calculate profit
                ...
                trades.append({
                    'entry_date': entry_date_val.strftime('%Y-%m-%d') if hasattr(entry_date_val, 
'strftime') else str(entry_date_val),
                    'exit_date': exit_date_val.strftime('%Y-%m-%d') if hasattr(exit_date_val, 
'strftime') else str(exit_date_val),
    ...
    try:
        ...
        if 'daily_return' not in df.columns:
            if 'equity' in df.columns:
                ...
                logger.warning("No equity column found in signals_df for advanced metrics 
calculation")
                return metrics
        ...
        total_days = len(df)
        metrics['percent_profitable_days'] = (profitable_days / total_days) * 100 if total_days > 0 
else 0
        ...
        if 'trade_profit' in df.columns:
            ...
            if not trades.empty:
                ...
                # Win rate
                metrics['win_rate_percent'] = (win_count / total_trades) * 100 if total_trades > 0 
else 0
                
                # Calculate profit factor
                total_profit = winning_trades['trade_profit'].sum() if not winning_trades.empty 
else 0
                total_loss = abs(losing_trades['trade_profit'].sum()) if not losing_trades.empty 
else 0
                metrics['profit_factor'] = total_profit / total_loss if total_loss > 0 else (1.0 if 
total_profit == 0 else 10.0)
                ...
                metrics['annual_return_percent'] = (((end_equity / start_equity) ** (1/years)) - 1) 
* 100 if years > 0 else 0
        ...
        if metrics.get('annual_return_percent', 0) == 0 and metrics.get('total_return_percent', 0) 
> 0:
...
# Add a helper function at the end of the file to calculate equity if missing
def calculate_equity_curve(signals_df, initial_capital=10000.0, commission=0.001):
    """
    Calculate equity curve from signals DataFrame if not already present.
    ...
    Returns:
        pd.DataFrame: DataFrame with equity column added
    """
    df = signals_df.copy()
    
    # Skip if equity already calculated
    if 'equity' in df.columns and not df['equity'].equals(initial_capital):
        return df
    ...
    for i in range(len(df)):
        ...
        if current_signal == 'buy' and position == 0:
            ...
            df.at[df.index[i], 'position'] = position
            df.at[df.index[i], 'entry_price'] = 0.0
            df.at[df.index[i], 'equity'] = equity
        else:
            df.at[df.index[i], 'position'] = position
            df.at[df.index[i], 'entry_price'] = entry_price
...
# Add this function near the calculate_equity_curve function to help debugging
def debug_dataframe_info(df, name="Dataframe"):
    """Log detailed information about a DataFrame to help with debugging."""
    if df is None:
    ...
    try:
        ...
        logger.info(f"{name} index type: {type(df.index)}")
        logger.info(f"{name} first/last dates: {df.index[0]} to {df.index[-1]}")
        ...
        if 'equity' in cols:
            equity_stats = {
                ...
                'change': df['equity'].iloc[-1] - df['equity'].iloc[0]
            }
            logger.info(f"{name} equity stats: {equity_stats}")
        
        if 'signal' in cols:
            signal_counts = df['signal'].value_counts().to_dict()
            logger.info(f"{name} signal counts: {signal_counts}")
            
    except Exception as e:
        logger.error(f"Error analyzing {name}: {str(e)}")


# Right after retrieving default_signals and optimized_signals:

# Add debug logging for default strategy signals
logger.info("Debug info for default strategy signals:")

# Add debug logging for optimized strategy signals
logger.info("Debug info for optimized strategy signals:")
...
if default_signals is not None and optimized_signals is not None:
    ...
    if len(common_dates) > 0:
        logger.info(f"Date range for comparison: {common_dates[0]} to {common_dates[-1]}")
